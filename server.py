...
window.addEventListener('resize', () => fit.fit());
    ...
    def list_directory(self, path):
        ...
        styled = (
            ...
            + '.rn-btn:hover{background:rgba(255,255,255,.2)}.vthumb,.vpic{width:44px;height:44px;object-fit:cover;border-radius:4px;display:block}'
            ...
            + 'function genVThumbs(){'
            + 'document.querySelectorAll("video.vthumb:not([data-vdone])").forEach(function(v){'
            + 'v.setAttribute("data-vdone","1");'
            + 'v.addEventListener("loadedmetadata",function(){'
            + 'var t=this.duration*0.4;if(t>0){this.currentTime=t}'
            + '});'
            + 'v.addEventListener("seeked",function(){'
            + 'var c=document.createElement("canvas");c.width=this.videoWidth||88;c.height=this.videoHeight||88;'
            + 'var ctx=c.getContext("2d");ctx.drawImage(this,0,0,c.width,c.height);'
            + 'var img=document.createElement("img");img.className="vpic";img.src=c.toDataURL();'
